...
Follow-up to 2.2.1 / 2.3.0. Reported by Rick (SiTech II controller on Mathis MI500/750, 16″ GSO RC, 17×24′ FOV): with **Skip solve** enabled, even after 2.2.1's "use current mount position as ring center" change, the first ring slew still moved his manually-centered star several arcmin off-axis and pushed the 80%-of-FOV ring partially outside the sensor. Root cause was an epoch double-precession in our own code, not a NINA or mount-driver bug.

### Fixed
- **Skip solve no longer double-precesses the current mount position** — the Skip-solve path constructed `Coordinates(info.RightAscension, info.Declination, Epoch.J2000)` from the raw ASCOM driver values. But `TelescopeInfo.RightAscension / .Declination` are reported in the driver's declared `EquatorialSystem` (typically JNOW for most ASCOM mounts), **not** J2000. Labelling them as J2000 ourselves meant that NINA's `SlewToCoordinatesAsync` — which internally calls `coords.Transform(EquatorialSystem)` to precess J2000→native before sending RA/Dec to the driver — was applying a one-way precession with no inverse, so every downstream slew landed at `native_pointing + precession_delta`. At typical declinations 26 years past J2000.0 this is roughly 19′ RA and 5–8′ Dec — exactly the "off to the side" symptom Rick observed. The plugin now reads `info.Coordinates` (already correctly tagged with the driver's `EquatorialSystem`) and calls `Transform(Epoch.J2000)` to convert it properly before storing it as `ringCenterCoords`. Subsequent J2000→native precession at each slew now inverts our Transform symmetrically and the mount returns to its starting pointing for the "Center" ring position, with ring offsets applied around it as intended. Default behaviour (Skip solve off → plate-solve + sync) was already epoch-safe via the `Sync` call after `Center` and is unchanged.
- **"Use Mount" panel button had the same double-precession bug** — it loaded raw `info.RightAscension / .Declination` into the `TargetRA` / `TargetDec` fields, which downstream wraps as `Coordinates(..., Epoch.J2000)` for plate-solve, blind slew, and visual map. Anyone using the "Use Mount" button on a JNOW-reporting driver was silently working with a J2000-coordinate set offset by the precession delta from what the mount actually saw — masked by plate-solve when Skip solve was off, but visible in the on-screen map and in any subsequent slew that bypassed the solver. The button now applies `info.Coordinates.Transform(Epoch.J2000)` before string-formatting, so what the panel shows and what NINA slews to are both true J2000 of the current pointing. Status text now includes the `(J2000)` tag to make the epoch explicit.

### Why most users never saw this
- The standard path (Skip solve off) calls NINA's `Center` instruction, which plate-solves the actual sky and syncs the mount — that physically anchors the scope on the star regardless of what epoch the driver claims, masking the bug.
- High-end mounts whose ASCOM driver declares `EquatorialSystem = J2000` (some 10Micron modes, some SiTech firmware) skip NINA's precession entirely on slew, so no double-transform occurs.
- Users with wide-field rigs (≥ 1° FOV) absorbed the 10–20′ offset inside their ring radius and didn't notice ring-distribution skew.
- Rick's specific combination — small FOV, sparse star field forcing Skip solve, JNOW-reporting SiTech driver — was the worst-case intersection that surfaced it.
