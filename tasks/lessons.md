...
## [2026-05-25] — Never label raw mount RA/Dec as J2000 — Transform it first
- **Mistake:** When reading the mount's live pointing (`UseMountPosition` button and the Skip-solve runtime path in `RunCollimation`), the plugin wrapped `info.RightAscension / info.Declination` directly in `new Coordinates(..., Epoch.J2000)`. Reported by Rick (SiTech II controller on Mathis MI500/750, 16″ GSO RC, 17×24′ FOV): every Skip-solve run still moved his manually-centered star several arcmin off-axis after v2.2.1 claimed to fix it. v2.3.0 inherited the bug unchanged.
- **Root cause:** `TelescopeInfo.RightAscension / .Declination` are RAW ASCOM driver values in the driver's declared `EquatorialSystem` (typically JNOW for most mounts; J2000 only when the driver explicitly reports it). Labelling them as J2000 ourselves and then calling `SlewToCoordinatesAsync(coords)` triggers NINA's internal `coords.Transform(EquatorialSystem)` — a one-way J2000→native precession with no inverse applied to our input. Result: the mount slews to `native_pointing + precession_delta` (≈ 19′ RA / 5–8′ Dec at typical declinations 26 years past J2000.0). NINA *does* expose `TelescopeInfo.Coordinates`, which is already correctly stamped with the driver's epoch — but I didn't use it. The fact that v2.2.1's commit message rationalised the bug ("epoch drift here is masked by the fact that ALL ring slews share the same reference frame") made it sit unnoticed through v2.3.0.
- **Rule:** Never construct a `Coordinates` object from raw `TelescopeInfo.RightAscension / .Declination` without epoch-tagging from `info.EquatorialSystem` or — preferred — going through `info.Coordinates` and `.Transform(Epoch.J2000)` to canonicalise. Treat `info.RightAscension / .Declination` as **opaque driver-native scalars**, not as J2000. Verify any rationalisation in a commit message that claims epoch handling "cancels out" or "doesn't matter" by tracing the actual `Transform` calls inside NINA's slew pipeline — they almost never cancel out cleanly when only one side is mis-labelled.
- **Applies to:** Every code path that reads live mount pointing — `UseMountPosition`, Skip-solve, future "snap ring to current position" features, any sequence item that synthesises a ring center from live mount state instead of a cataloged target. Also general lesson for NINA plugin work: the canonical "where is the mount pointing in J2000?" is `info.Coordinates.Transform(Epoch.J2000)`, NOT `(info.RightAscension, info.Declination)` cast to J2000.

## [2026-05-09] — Plate-solve must be opt-out, not mandatory, for sparse-field targets
- **Mistake:** `RunCollimation` always called NINA's `Center` (plate-solve) instruction with no way for the user to bypass it. User Rick (16″ GSO RC, 17×24′ FOV, Nekkar) hit a hard wall: his plate-solver couldn't solve the centring frame on a sparse, bright-mag-3-dominated field, so the run aborted before defocus, no matter which solver he tried. The pre-existing `catch (Exception psEx)` fallback to a blind slew only triggers when `Center` raises — but in his setup `Center` was retrying internally and reporting failure as a hard error path that abort the run before the catch could degrade gracefully, OR the user simply wanted to skip the step entirely without waiting for it to fail.
- **Root cause:** The plugin treated plate-solve as load-bearing for correctness (it does mask mount epoch bugs) and offered no opt-out. But "load-bearing for correctness" ≠ "load-bearing for completing the run". Users with reliable mount pointing and a correctly-reported epoch driver have no need for plate-solve, and forcing it on them turns a known-good slew into a hard failure when the local solver can't handle a sparse field.
- **Rule:** When a step is "highly recommended but not strictly required for the run to produce useful output", expose it as a user-visible toggle (default ON), not a hardcoded path. Both branches of the toggle must surface their correctness implications: the ON branch via its own logging and the OFF branch via a `Notification.ShowWarning` + persistent `WARNING:`-style StatusText. Never silently degrade — always announce, regardless of whether the user opted in or the system fell back.
- **Applies to:** SkwPanelVM `SkipCentering` toggle; future togglable steps where the default protects against subtle correctness bugs (mount sync, autofocus-before-defocus, filter restoration on cancel).
